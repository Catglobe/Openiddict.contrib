...
public static class AuthorizationCodeHelpers
{
   ...
   /// <param name="model">Current controller.</param>
   /// <returns>Action to take, or null to continue processing.</returns>
   public static IActionResult? ReAuthenticateIfNecessary(this AuthenticateResult result, OpenIddictRequest request, PageModel model)
   {
      ...
      return model.Challenge(new AuthenticationProperties {RedirectUri = httpRequest.PathBase + httpRequest.Path + QueryString.Create(parameters)});
   }
   ...
   public static ForbidResult ForbidOpenIddict(this PageModel model, string errorType, string errorText) =>
      model.Forbid(authenticationSchemes: OpenIddictServerAspNetCoreDefaults.AuthenticationScheme,
   ...
   public static OpenIddictRequest GetOpenIddictServerRequest(this PageModel model) =>
      model.HttpContext.GetOpenIddictServerRequest() ??

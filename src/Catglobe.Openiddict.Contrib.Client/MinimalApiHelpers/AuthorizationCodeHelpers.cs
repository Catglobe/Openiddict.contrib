...
namespace Openiddict.Contrib.Client.MinimalApiHelpers;
...
public static class AuthorizationCodeHelpers
{
   ...
   ///    return Results.LocalRedirect($"/missing-permissions?missing={HttpUtility.UrlEncode(string.Join(",", missingPermissions))}");
   ///  }
   /// </code></example>
   /// </param>
   ...
   /// </param>
   ///  <returns>The result you need to return from the controller</returns>
   public static async Task<IResult> StoreRemoteAuthInSchemeAsync(this HttpContext httpContext, Func<ClaimsPrincipal, IResult?>? customCheck = null, Action<ClaimsIdentity, ClaimsPrincipal>? storeRemoteInfo = null)
   {
     // Retrieve the authorization data validated by OpenIddict as part of the callback handling.
      ...
      if (resultPrincipal is not { Identity.IsAuthenticated: true })
      {
   ...
   /// <example>For example:
   /// <code>
   /// host.MapGet("/logout-oidc", [AllowAnonymous](HttpContext httpContext, string returnUrl = "/") =&gt; httpContext.LocalAndRemoteLogOutAsync(returnUrl));
   /// host.MapGet("/logout/callback", (HttpContext httpContext, string? _ = null) =&gt; httpContext.RemoteLogOutCallbackAsync());
   ...
   /// <param name="returnUrl">Parameter from UI where the user will be redirected after the auth is done</param>
   /// <returns>The result you need to return from the controller</returns>
   public static async Task<IResult> LocalAndRemoteLogOutAsync(this HttpContext httpContext, string returnUrl)
   {
   ...
   /// <param name="httpContext">The controller</param>
   /// <returns>The result you need to return from the controller</returns>
   public static async Task<IResult> RemoteLogOutCallbackAsync(this HttpContext httpContext)
   {
      // Retrieve the data stored by OpenIddict in the state token created when the logout was triggered.
      var result = await httpContext.AuthenticateAsync(OpenIddictClientAspNetCoreDefaults.AuthenticationScheme);

      return Results.LocalRedirect(result?.Properties?.RedirectUri ?? "/");
   }

   /// <summary>
   /// Helper function, if you want to be sure that there is no active login.
   /// <example><code>
   ...
   /// </code></example>
   /// </summary>
   public static async Task<IResult> EnsureLoggedOut(this HttpContext httpContext, string localUrl)
   {
      ...
      if (result is not { Succeeded: true }) { goto redirectToPage; }
      ...
      if (result.Properties.Items.TryGetValue(OpenIddictClientAspNetCoreConstants.Properties.ProviderName, out var provider)) properties.Items[OpenIddictClientAspNetCoreConstants.Properties.ProviderName] = provider;

      await httpContext.SignOutAsync(OpenIddictClientAspNetCoreDefaults.AuthenticationScheme, properties);
      await httpContext.SignOutAsync();
